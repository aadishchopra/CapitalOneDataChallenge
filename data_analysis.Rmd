...
trans_zil$ds=seq.Date(from = as.Date('1996/04/01'),to = as.Date('2017/06/01'),by = 'month')

bind_for_prophet<-apply(trans_zil,2,cbind.data.frame,trans_zil['ds'])
bind_for_prophet<-lapply(temp, setNames,c('y','ds'))
make_model<-lapply(temp1, prophet)
forecast<-lapply(ex_yhat, predict,data.frame(ds="2018-05-01"))

lapply(make_model, dyplot.prophet,forecast)
