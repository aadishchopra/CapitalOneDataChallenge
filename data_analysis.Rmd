...
no_of_bedrooms<-2
two_room_property<-
  AirBNB %>% filter(AirBNB$bedrooms ==no_of_bedrooms)

how_many_NA_2_bedrooms<-as.data.frame(unlist(apply(two_room_property,MARGIN =  2, noofNA)))


two_room_property %>% select(zipcode,price) %>% filter(zipcode>0)%>% group_by(zipcode)%>% summarise(avg_price=mean(price,na.rm = T),count=n()) %>%arrange(desc(count),desc(avg_price))%>%mutate_if(is.numeric,round,digits=0)%>% top_n(n = 10)%>% ggplot(.,mapping = aes(zipcode,avg_price))+geom_bar(stat = "identity")
